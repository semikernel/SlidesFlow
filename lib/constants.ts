import { HomeIcon } from "@/components/icons/icon"; // 假设 icon.tsx 和 data.ts 在同一级目录
import { PptIcon } from "@/components/icons/icon";
...
export const data = {
  user: {
    name: "Shancn",
    email: "shancn@163.com",
    avatar: "/avatars/shacn.jpg",
  },
...
export const containerVariants = {
  hidden: { opacity: 0 }, // 初始状态：透明度为 0（不可见）
  visible: {
    opacity: 1, // 结束状态：透明度为 1（完全可见）
    transition: {
      staggerChildren: 0.1, // 设置子元素动画延迟的时间，单位为秒
    },
  },
};

export const itemVariants = {
  hidden: { y: 20, opacity: 0 }, // 初始状态：元素在 Y 轴偏移 20px，并且透明度为 0（不可见）
  visible: {
    y: 0, // 动画结束状态：恢复到正常位置，Y 轴偏移为 0
    opacity: 1, // 完全可见，透明度为 1
    transition: {
      type: "spring", // 动画类型为 "spring"，即弹簧效果
      stiffness: 100, // 设置弹簧的刚度，影响动画的紧致程度
    },
  },
};
